<?php 
	...
	if (isset($_FILES['files']['size']) && $_FILES['files']['size'] == 0) {
		$_SESSION['error']['size'] = "Veuillez selectionner une image";
		header("Location: index.php");
		die();
	}
	...
	if (!empty($_FILES)) {
		...
		$new_title = !empty($_POST['upload_title']) ? $_POST['upload_title'] : null;
		...
		$title = (!empty($_POST['upload_title']) ? $_POST['upload_title'] : $title)."_".rand(1, 100).'.'.time();
		// pourquoi tu a mis rand.time plutot que rand_time ?
        if (preg_match('/(.+\.jpeg|.+\.jpg|.+\.png)$/i', $img['name']) && $img['size'] <= MAX_IMG_SIZE) {
			move_uploaded_file($img['tmp_name'], $user_folder.$title.".png");
			Helper::getDB()->query(
				"INSERT INTO pictures SET user_id=:user_id, title=:title, description=:description, created=:created, file=:file, like_count=0;", 
				array(
